	...
	public void perform(Execution execution){
		...
		for (int i = 0; i<routines.length; i++){
			...
			if ("exit".equals(routines[i].toLowerCase())){
				...
				Util.log("forward => currentOption:"+currentOption);
				
				execution.variables.put("currentOption", currentOption);
			}
			if ("double-exit".equals(routines[i].toLowerCase())){
				Option currentOption = (Option) execution.variables.get("currentOption");
				...
				execution.variables.remove("SequenceIndex_"+((Sequence) currentOption).currentId);
				...
				Util.log("atribution: "+variable + " = " +expression);
