	...
	public String getMenu(Execution execution){
		...
		while (iterHeader.hasNext()){
			...
			if ("text".equals(hyperText.type))
				...
				Util.log("menu-hyperTextContent: "+hyperTextContent);
		...
		while (iterOptions.hasNext()){
			...
			if (canAddOption){
				...
				String currentSecretKey = XMLZap.applyContext(execution, secretKey);
				
				//Util.log(order+" secretKey:"+currentSecretKey+";"+(!"".equals(currentSecretKey))+";"+( !keyText.contains(currentSecretKey)));
				if ("".equals(currentSecretKey)){
					result += "\n" + keyText;
					order++;
				}
				else if (!keyText.contains(currentSecretKey)){
					result += "\n" + keyText;
					order++;
				}
			}
		...
		while (iterFooter.hasNext()){
			...
			if ("text".equals(hyperText.type))
				...
				Util.log("hyperTextContent: "+hyperTextContent);
				Messenger.sendMedia((String) execution.variables.get("jidServer"), 
									(String) execution.variables.get("jidClient"),
									(String) execution.variables.get("jidUrlClient"),
									((String) execution.variables.get("currentPath")) +  hyperTextContent, 
		...
		if (includeBackOption)
			result += "\n\n"+execution.variables.get("currentReturnText");
