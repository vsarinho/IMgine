...
public class XMLParser {	
	...
	public static void main(String[] args) throws Exception {
		...
		Option menu = loadXMLConfig("/home/everton/projects/IMgine/MyTicTacToe/config.xml"/*"<my_zapapp_config.xml>"*/, teste);
		
		Util.log("path:"+teste.currentPath + "\n" + menu);
	}
	...
	public static Option makeTree(Node root){
		if (debug) Util.log("makeTree "+root.getNodeName());
	...
	public static Menu makeMenu(Option menuParent, Node root){
		if (debug) Util.log("makeMenu "+root.getNodeName());
    	...
    	result.secretKey = getNodeAttr("secretKey", root);
    	
	    return result;
	}
	
	public static Option makeCommand(Option currentParent, Node root){
		if (debug) Util.log("makeCommand "+root.getNodeName());
	    ...
	    for (int x = 0; x < nodes.getLength(); x++ ) {
	        ...
	        if (node.getNodeName().equalsIgnoreCase("menu")) {	 
	        	menuNode = node;
	        	if (debug) Util.log("isMenu!!");
	        }
	        else if (node.getNodeName().equalsIgnoreCase("sequence")) {	 
	        	sequenceNode = node;
	        	if (debug) Util.log("isSequence!!");
	        }
	    }
    	Option result = null;
	    ...
	    if (sequenceNode != null)
		    ...
		    for (int x = 0; x < nodes.getLength(); x++ ) {
		        ...
		        if (node.getNodeName().equalsIgnoreCase("image") || 
	        		...
	        		node.getNodeName().equalsIgnoreCase("http-text") || 
	        		node.getNodeName().equalsIgnoreCase("http-image") || 
					node.getNodeName().equalsIgnoreCase("http-document") || 
		        	node.getNodeName().equalsIgnoreCase("text")) {	
	...
	public static Option makeSequence(Option currentParent, Node root){
		if (debug) Util.log("makeSequence "+root.getNodeName());
    	...
    	for (int x = 0; x < nodes.getLength(); x++ ) {
			...
			else if (node.getNodeName().equalsIgnoreCase("wait")) {
	        	result.options.add(makeWait(result, node));
	        }
	        else if (node.getNodeName().equalsIgnoreCase("menu")) {	        	
	        	result.options.add(makeMenu(result, node));
		    }
    	...
    	result.description = getNodeAttr("description", root);
    	result.condition = getNodeAttr("condition", root);
    	result.isLooping = getNodeAttr("isLooping", root);
    	
    	if (result.isLooping == null || "".equals(result.isLooping))
	...
	public static HyperText makeHyperText(Node root){
		if (debug) Util.log("makeHyperText "+root.getNodeName());
	...
	public static Exec makeExec(Node root){
		if (debug) Util.log("makeExec "+root.getNodeName());
	...
	public static Prompt makePrompt(Option menuParent, Node root){
		if (debug) Util.log("makePrompt "+root.getNodeName());
	...
	public static Wait makeWait(Option menuParent, Node root){
		if (debug) Util.log("makeWait "+root.getNodeName());
